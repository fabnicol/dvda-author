...
pkgdatadir = $(datadir)/lplex
pkgincludedir = $(includedir)/lplex
pkglibdir = $(libdir)/lplex
pkglibexecdir = $(libexecdir)/lplex
...
am__aclocal_m4_deps = $(top_srcdir)/m4/libFLAC++.m4 \
	$(top_srcdir)/m4/libFLAC.m4 $(top_srcdir)/m4/libtool.m4 \
...
am__installdirs = "$(DESTDIR)$(lplexdocdir)"
DATA = $(lplexdoc_DATA)
...
OTOOL64 = 
PACKAGE = lplex
PACKAGE_BUGREPORT = audioplex-lpcm@lists.sourceforge.net
PACKAGE_NAME = lplex
PACKAGE_STRING = lplex 0.3
PACKAGE_TARNAME = lplex
PACKAGE_URL = 
PACKAGE_VERSION = 0.3
PATH_SEPARATOR = :
RANLIB = ranlib
...
install-lplexdocDATA: $(lplexdoc_DATA)
	@$(NORMAL_INSTALL)
	@list='$(lplexdoc_DATA)'; test -n "$(lplexdocdir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(lplexdocdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(lplexdocdir)" || exit 1; \
	fi; \
	for p in $$list; do \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(lplexdocdir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(lplexdocdir)" || exit $$?; \
	done

uninstall-lplexdocDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(lplexdoc_DATA)'; test -n "$(lplexdocdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(lplexdocdir)'; $(am__uninstall_files_from_dir)
...
installdirs-am:
	for dir in "$(DESTDIR)$(lplexdocdir)"; do \
...
install-data-am: install-lplexdocDATA
	@$(NORMAL_INSTALL)
...
uninstall-am: uninstall-lplexdocDATA
...
.PHONY: $(am__recursive_targets) CTAGS GTAGS TAGS all all-am \
	...
	install-lplexdocDATA install-man install-pdf install-pdf-am \
