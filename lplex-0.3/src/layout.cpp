    ...
    if (fail)
        {
            ...
            for (int i = 0; i < 3; i++)
                if (fail & (0x01 << i))
                    ERR("Not enough space on device " << device(devices[i])
                        << " (" << sizeStr(freeSpace) << " free)\n");
#else
            struct
            {
                dev_t id;
            ...
            for (int i = 0; i < 3; i++)
                if (fail & (0x01 << i))
                    {
                        //  ERR( string(devices[i].path) +
                        //  string(": not enough space on device ")
                        //  + string(device( devices[i].id )))

                        ERR(" - " + string(sizeStr(devices[i].space))
                                  + string(" free )\n"));
                    }
#endif
            LOG("\n");
            ...
            if (job->prepare >= vobf || job->params & dvdStyler)
                {
                    ...
                    LOG(_f("                        + %s (\'workpath\'%s\n",
                           sizeStr(total.estimate *
                                   (job->params & cleanup ? 1 : 2)).c_str(),
                           job->params & cleanup ?
                           (job->prepare >= isof ?
                            " and \'isopath\', shared)" : ")") : ")"));
                    ...
                    if (w == n)
                        {
                            LOG("\n");
                            LOG("-setting \'cleanup\' to \'true\' "
                                "will reduce required space to "
                                << sizeStr(total.estimate * 2) << ".\n");
                        }

                    LOG("\n");
                    LOG("-you can redirect \'dvdpath\', \'workpath\',"
                        "or \'isopath\' by either\n");
    ...
    for (uint i = 0; i < Lfiles->size(); ++i)
        {
            ...
            if (i == Lfiles->size() - 1
                    || (i < Lfiles->size() - 1
                        && (*Lfiles)[i + 1].group != titleset))
                {
                    lFile->type |= lpcmFile::titleEnd;
                }

            if (i < Lfiles->size() - 1
                && (*Lfiles)[i + 1].trim.type != lFile->trim.type)
                {
            ...
            if (!(lFile->type & lpcmFile::seqStart) &&
                    (lFile->trim.type & (jobs::continuous
                                         | (jobs::continuous << 4))))
            ...
            if (lFile->trim.type & jobs::discrete)
            ...
            else if (lFile->trim.len != audioBytes)
                {
                    ...
                    if (lFile->trim.shift & jobs::backward ||
                            (lFile->trim.shift & jobs::nearest
                               && abs(orphans) > (lFile->trim.unit / 2)))
                        {
            ...
            if (lFile->type & lpcmFile::titleStart)
                {
                    ...
                    LOG("t:  original : realigned : offset :  pad   : loss :  "
                        "gap   :     frames     :  timestamp  :jpg: file\n");
                    LOG(".:...........:...........:........:........:......:.."
                        "pts...:....A...:...V...:.............:.#.:.........\n");
                }
            ...
            char p[100];
            stringstream x;
            x << setw(10) << audioBytes
              << " :" << setw(10) << lFile->trim.len << endl;
            x.getline(p, 100);

            lFile->details = _f("%s:%s :%7d :%7d :%5d%s:%7d"
                                "%s:%7d :%6d : %s :%2d",
                                (lFile->type & lpcmFile::seqEnd) ?
                                "e" :
                                ((lFile->trim.type & jobs::discrete) ?
                                 ((lFile->trim.type & jobs::padded) ?
                                   "p" : "d"):
                                 (lFile->trim.type & jobs::seamless) ?
                                   "s" : "n"),
                                p,
                                lFile->trim.offset,
                                audioPadding,
                                audioLoss,
                                (audioLoss ? "*" : " "),
                                audioGap + (uint16_t)(150 *
                                                      ((float)audioPadding
                                                       / (float)dvdAudioFrame)),
                                (lFile->trim.type
                                 & (jobs::discrete | jobs::notrim)) ? "+" : " ",
                                audioFrames,
                                lFile->videoFrames,
...
//     ( editing ? lFile->fName.string() : lFile->fName.filename() ) << endl );
            ...
            if (lFile->type & lpcmFile::titleEnd)
                {
                    LOG(_f("[col 1] -trim units: s(AVseam)=%d"
                           " d,e(audioFrame)=%d n(sampleSeam)=%d bytes.\n",
                           dvdAVseam, dvdAudioFrame, dvdSampleSeam));
                }

            titleVframes += lFile->videoFrames;
            total.video += m2vEstimate(lFile/*->videoFrames*/,
                                       job->tv == NTSC);
    ...
    ECHO("\n");
    LOG("Legend:\n");
    LOG("[col 1] -trim types: (s)eamless (d)iscrete"
        " (e)ndOfSequence (p)added (n)one.\n");
    if (note & discon)
        LOG("(+)     -dvdauthor's STC discontinuity will add"
            " a half-second pause between chapters.\n");
    if (note & loss)
        LOG("(*)     -bytes will be discarded as"
            " 'incomplete final frame' by mplex.\n");
    if (note & padding)
...
//      SCRN( INFO_TAG << TINT( _f( "Title %d - (%s / %s %s)\n\n",
            ...
            SCRN(TINT(_f("Title %d - (%s / %s %s)\n\n",
                         writeFile->group + 1,
                         lpcmEntity::audioInfo(writeFile).c_str(),
            ...
            if (reader->state & lpcmEntity::_eof)
                {
                    if (writeIndex == readIndex
                        && writeFile->trim.type & jobs::discrete)
                        finished = true;

                    if (finished)
                        {
                            padding = writeFile->trim.len
                              - (total.now + reader->ct.now);
                            if ((samplePadding = (total.now + reader->ct.now) %
                                                 sampleSeam(&reader->fmeta)))
                                {
                                    memset(bigBlock + reader->ct.now,
                                           '\0', samplePadding);
    ...
    md5_finish(&md5sum,
               (md5_byte_t *) &writeFile->fmeta.data.stream_info.md5sum);
...
#else
//            xml << _f( "<dvdstyler name=\"%s\" videoFormat=\"%d\" "
//     "aspectRatio=\"1\" audioFormat=\"%d\" videoBitrate=\"-1\" "
//            "capacity=\"0\" emptyMenu=\"1\" format=\"4\" jumppad=\"1\">\n\n",

                xml << _f("<dvdstyler name=\"%s\" videoFormat=\"%d\" "
                          "aspectRatio=\"%d\">\n\n",
                          fs::path(str).stem().c_str(), video == NTSC ? 3 : 2,
                          flag ? 2 : 1);
#endif
...
//            xml << "\t<vmgm />\n\n";
// nonsense, but otherwise dvdauthor 0.7.0 fails with
// "no video format specified for VMGM"

                xml << _f("\t<vmgm><menus><video format=\"%s\"/>"
                          "</menus></vmgm>\n\n", video == NTSC ? "ntsc" : "pal");
...
#endif
                            << "\t\t\t\t<post>"
                            << _f("if(g1 eq %d){g1=%d; jump title 1;}"
                                  " jump vmgm menu;",
                                  titleset, titleset + 1)
                            << "</post>\n"
                            << "\t\t\t</pgc>\n"
...
#else
                    << (dvdStyler ? _f("\t\t\t\t<vob><menu videoFormat=\"%s\">"
                                       "</menu></vob>\n",
                                       video == NTSC ?
                                       "NTSC" : "PAL").c_str() : "")
#endif
