...
inline static void test_field(uint8_t* tab__, uint8_t* tab, int size,const char* label, FILE* fp, _Bool write)
{
    ...
    {
        if (write) fwrite(tab__, size, 1, aob_log);
        if (globals.logdecode)
            fprintf(aob_log, "%s", ";\n");
    }
}
...
inline static void rw_field(uint8_t* tab, int size,const char* label, FILE* fp)
{
    ...
    if (globals.logdecode) fprintf(aob_log, "%" PRIu64 ";%s;", offset, label);
    fwrite(tab, size, 1, aob_log);
    if (globals.logdecode) fprintf(aob_log, "%s", ";\n");
}
...
inline static void read_audio_pes_header(FILE* fp, uint16_t PES_packet_len, uint8_t extension_flag, uint32_t* PTS_ptr)
{
    uint8_t packet_start_code_prefix[3]={0x00,0x00,0x01};
    uint8_t stream_id[1]={0xbd}; // private_stream_1
    uint8_t PES_packet_len_bytes[2];
    ...
    uint8_t PES_packet_len_bytes__[2]={0};
    ...
    CHECK_FIELD(PES_packet_len_bytes);
    CHECK_FIELD(flags1);
    CHECK_FIELD(flags2);
...
inline static int read_lpcm_header(FILE* fp, fileinfo_t* info, int64_t pack_in_title, uint8_t counter, uint8_t position)
{
    uint8_t sub_stream_id[1]={0xa0};
    uint8_t continuity_counter[1]={0x00};
    uint8_t LPCM_header_length[2];
    ...
    uint8_t continuity_counter__[1]={0};
    ...
    /* offset_count += */   CHECK_FIELD(continuity_counter);
...
inline static int read_pes_packet(FILE* fp, fileinfo_t* info, uint8_t* audio_buf)
{
    ...
    uint header_length = read_lpcm_header(fp,info,pack_in_title,cc, position);
    ...
    switch(position)
    {
      case FIRST_PACK :
        ...
        read_audio_pes_header(fp,info->firstpack_audiopesheaderquantity, 1, &PTS); //+17
        break;

      case MIDDLE_PACK :
        audio_bytes=info->lpcm_payload;

        read_audio_pes_header(fp,info->midpack_audiopesheaderquantity, 0, &PTS); //+14
      ...
      case LAST_PACK :
        //audio_bytes = bytesinbuffer;
        // to be implemented!
        read_audio_pes_header(fp,info->lastpack_audiopesheaderquantity+audio_bytes, 0, &PTS); //+17
        break;
    }


    /* 14 + 3 + 4 + header_length = 21 + header_length = offset1-offset */

    if (21 + header_length != offset1 - offset)
    {
        if (globals.logdecode)
        {
            open_aob_log();
    ...
    switch(position)
    {
      case FIRST_PACK :
        if (info->firstpack_pes_padding > 0)
        {
           read_pes_padding(fp,info->firstpack_pes_padding);
        }
        break;
      ...
      case MIDDLE_PACK :
        ...
        {
           read_pes_padding(fp,info->midpack_pes_padding);
        }

        break;

      case LAST_PACK :
                read_pes_padding(fp,2006-audio_bytes);
        break;
    }
...
int decode_ats(char* audiotsdir,int titleset,fileinfo_t* files)
{
    ...
    do
    {
        ...
        n=read_pes_packet(fp, &files[i], audio_buf);
