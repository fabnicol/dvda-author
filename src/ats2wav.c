...
static const uint8_t  T[2][6][36] =
{
  { {0}, // 4
    ...
    {13, 12, 15, 14, 17, 16, 1, 0, 3, 2, 5, 4, 19, 18, 21, 20, 23, 22, 7, 6, 9, 8, 11, 10}
  }, //rev
  { {4,  1,  0,  5,  3,  2},
  ...
  if (buff[0] == 0 && buff[1] == 0 && buff[2] == 1 && buff[3] == 0xBA && buff[4] == 0x44)
    {
      ...
      else if (buff[0x15] == 0x80 && buff[0x27] == 0x80 ||  buff[0x27] == 0x81 && buff[0x3C] == 0x80) //  middle-last pack  || first pack
        {
...
static void open_player(const WaveData *info, const WaveHeader  *header, globalData *globals)
{
  ...
  else if (strcmp(globals->player, VLC_BASENAME) == 0)
    {
      if (globals->player_path[0] == '\0')
          ...
          do
            {
              ...
              if (wav_numbytes == 0)
              ...
              else
                {
                  ...
                  else if (wav_numbytes == written_bytes)
                    {
